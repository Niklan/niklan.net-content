...
В прошлой статье я написал [как создавать собственные DisplaySuite поля в Drupal 7](/blog/84). После я решил посмотреть и разобраться как они работают в Drupal 8. И в этой статье я уже расскажу как создать те же самые поля, но в реалиях Drupal 8.
...
Если оно у вас не появилось, сбросьте кэш `drush cr` или перейдите по url `/core/rebuild.php`. Если вы не отключали кэширование, советую [сделать отключение кэширование бэкенда](https://github.com/Niklan/Trash/blob/master/Drupal/8.x/disable_cache.md).
