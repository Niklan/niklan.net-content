...
опыт будет лучше, если посетитель будет попадать на горячий кеш с TTFB <10мс,
нежели холодный TTFB 1000мс (как пример). В идеале, пользователь должен всегда
...
на [очередях](/blog/79) и [плагинах](/blog/134) `@Warmer`. Также модуль
предоставляет команды для Drush и административный интерфейс, в котором можно
запускать прогрев вручную и настраивать плагины.
...
Модуль Warmer предоставляет только API, но в поставке также имеется два
...
одной операции [пакетной обработки данных](/blog/192). Иными словами, какое
...
использованием [@QueueWorker](/blog/200) и этот подход более щадящий для
сервера.
...
Так же держите в уме то, что [Queue Worker](/blog/200) плагины, выполняемые по
Drupal крону, обрабатывают очереди последовательно и в таком случае, либо нужно
выносить обработчики (свои или модуля) из крона и вешать вызов на системный крон
через Drush, либо учитывать общее время выполнение всех очередей, а не только от
Warmer модуля.
