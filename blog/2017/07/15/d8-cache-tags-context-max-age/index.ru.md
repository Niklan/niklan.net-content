...
Продолжая статью про [#lazy_builder](/blog/154) мы переходим к самому
...
2. Объявление данного объекта в качестве [сервиса](/blog/150). Данный сервис
...
- `getCachebaleMetadata()`: возвращает информацию о "кэшируемости" данного
  ...
  - Массив содержащий <mark>max-age = 0</mark>. Это означает что данный контекст
    ...
    используется [#lazy_builder](/blog/154), он будет задействован для
...
У меня есть материал про [Block Plugin API](/blog/75) и там нет такого метода.
Все потому что он не является частью этого API, он является частью Cache API и в
данной статье его самое время упомянуть. Если посмотреть в `BlockBase`, то он
также не содержит данных методов, но он расширяет `ContextAwarePluginBase`,
который и содержит данные методы.
