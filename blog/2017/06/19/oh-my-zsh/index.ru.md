...
- **vagrant** — так как я часто пользуюсь vagrant из-за [Drupal VM](/blog/147),
  то мне он тоже не помешает. Также как и npm добавляет поддержку
  автодополнения. Там же есть и **docker**.
- **composer** — в 8-ке очень активно [использую composer](/blog/130), а данный
  плагин опять же, добавляет автодополнение, и помимо этого различные полезные
  синонимы команд. Например `cu` = ` composer update`.
