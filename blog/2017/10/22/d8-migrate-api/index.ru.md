...
при [программном создании сущностей](/blog/82). Хотите указать статус для ноды?
Используйте `status: VAR`, хотите задать значение для `body`? Вам либо нужно
сразу записывать туда массив, так как поле требует массив с `value` и `format`,
либо задавать подобным способом: `'body/value'`. Об особенностях YAML в
миграциях будет позже.
