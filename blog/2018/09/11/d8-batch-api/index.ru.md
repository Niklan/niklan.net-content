...
Batch API построен поверх [Queue API](/blog/79), и по сути, является его полным
аналогом, с тем лишь отличием, что предназначен для UI. Если Queue API, как
правило, выполняется в фоне, то Batch API создает свой Queue и выполняет его
после запуска, отображая прогресс выполнения операции.

![Прогресс batch](image/batch-progress.png)

У меня уже есть материалы ([раз](/blog/133), [два](/blog/136)) с примерами
использования Batch API. Если вы их читали, я рекомендую ознакомиться с
материалов все равно. Так как тут описан более оптимальный и производительный
способ обработки большого кол-ва данных. Даже быстрее того что показан в
материале про ускорение импорта ?.
...
нужно - [мой материал про импорт из csv](https://niklan.net/blog/133). В нём мы
...
На данный способ обратил моё внимание xandeadx в
материале [Drupal 8: Импорт из CSV — оптимизация при больших объемах данных](/blog/136).
