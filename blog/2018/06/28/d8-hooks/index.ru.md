...
* **Исполняемые хуки** — данный тип хуков очень универсальный. Он может быть
  ...
  рода [Events](/blog/170), и да, некоторые хуки ведут себя как события и
...
* Слово **hook** заменяется на название модуля, в котором данный хук было решено
  ...
  использовать **hook**_some_example(), то вам нужно написать функцию **example
  **_some_example() и Drupal, обнаружив её, вызовет её, а вы сделаете что
  необходимо. Это уже будет зависить от того хука, который вы решили
  использовать.
