...
**Placeholder Strategy** — сервисы, отвечающие за стратегию [рендера](/blog/210)
плейсхолдеров.
...
PHP (в чистом виде) не ассинхронный, он выполняет код последовательно. Если в
...
это — [ленивые билдеры](/blog/154).

**Ленивый билдер** (`#lazy_builder`) — позволяет производить специальную
разметку в [рендер массивах](/blog/210) для тех частей сайта, где заранее
...
- `placeholder_strategy.big_pipe`: Стратегия, предоставляемая одноимённым
  ...
  соединение) не закрывается и в него «пушатся» специальные конструкции, по мере
  готовности рендера ленивых билдеров, которые заменяют плейсхолдеры на
...
это создать [сервис](/blog/150) [с меткой](/blog/204) `placeholder_strategy`,
который
реализует `\Drupal\Core\Render\Placeholder\PlaceholderStrategyInterface`.
...
по `?destination`, добавляем куку, добавляем [кэш-контекст](/blog/155), чтобы
ответ имел разное поведение в зависимости от наличия куки и отвечаем.
...
нету — включён. Зная это мы меняем поведение стратегии.
...
Для этого нам потребуется реулизовать эту логику, и в этом деле нам
...
серьезного искусственногоо замедления). Ответ друпала сократился с 6.9 секунд до
