...
Для того чтобы показать на примере, добавим немного [Form API](/blog/73),
...
1. [hook_theme()](/blog/151) (хуки тем) — позволяет регистрировать шаблон, какие
   ему нужны аргументы, его обработчики и использовать сколько угодно раз. Он
   используется для формирования HTML разметки на основе полученных данных из
   рендер массива.
1. `@RenderElement` — плагины, которые, чаще всего используются в связке
...
1. `@FormElement` — разновидность `@RenderElement`. Используется для создания
   рендер элементов для [Form API](/blog/73). Иными словами для элементов,
   которые могут запрашивать данные у пользователя.

В большинстве задач, кейсов и решений хватит `hook_theme()`. Не вдаваясь в
детали того, как это сделать, ведь [всё разжёвано отдельно](/blog/151), нашу
...
рендеринга [в конце обработки запроса](/blog/208) или тех местах, где это
необходимо. Вы можете сами пользоваться данным сервисом и получать готовую HTML
разметку для каких-то задач, например в [REST ресурсах](/blog/165).
...
- `$is_root_call = FALSE` — индикатор того, что рендер должен предоставить
  ...
  для [ленивого билдера](/blog/154) на значения. Это также означает, что это
  «последний рендер», а не рекурсивный.
...
1. Если значение в `#access` является
   ...
   его [кэш-данные](/blog/155) добавляются к результату, независимо от его
   результа. Если данный объект запрещает доступ, то сразу возвращается пустой
   результат рендера.
...
Текущий этап подготавливает стандартные [кэш контексты](/blog/155) для всех
элементов. Текущая операция производится только если `$is_root_call` установлен
в `TRUE` или задано значение `['#cache']['keys']` для свойства элемента.
...
Если для элемента задано свойство [#lazy_builder](/blog/154), то начинается его
проверка.
...
и [рендерится в самом конце](/blog/208), где получившийся плейсхолдер заменяется
на результат. Поведение замены в конечном итоге зависит от активной стратегии
замены._
...
вы можете прочитать [в отдельном материале](/blog/151)._
...
хуков (`#theme`, [hook_theme()](/blog/151)). Его мы рассмотрим отдельно, так как
обрабатываются они своим собственным сервисом.
...
при [рендере основного содержимого](/blog/208), чтобы сохранить значение для
заголовка в `#title`.
