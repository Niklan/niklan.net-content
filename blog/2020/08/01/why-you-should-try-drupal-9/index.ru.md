...
полностью [вести через Composer](/blog/130), без костылей и прослоек.
...
есть [сервисы и Dependency Injection](/blog/150).
...
Пожалуйста, [Inbound и Outbound обработчики](/blog/183) позволят вам сделать всё
...
коде у нас нет. Это решается при помощи [рендер массивов](/blog/210), которые
описывают требования и зависимости (этакий компонентный подход), а уже затем,
Drupal это превращает в HTML или что требуется проекту.
...
Таким образом, PHP код [выставляет требования и готовит данные](/blog/151), а
Twig их превращает в результат. Это также позволяет иметь различные оформления
...
пожалуйста — вы можете [переключать темы оформления](/blog/126) хоть по фазе
луны. Все остальные темы даже не узнают что произошёл рендер и не создадут
оверхеда.
...
объявлять и управлять библиотеками проекта. Библиотека — это именованный набор
CSS и/или JS файлов. Библиотеки подключаются к рендер массивам или шаблонам
только там где они нужны. Рендер массив или шаблон с библиотекой не
...
В Drupal также стандартизована работа с формами. [Form API](/blog/73) построен
поверх рендер массивов и предоставляет инструментарий для создания форм любой
сложности.
...
благодаря [гибкому и умному кэшированию](/blog/155). Drupal не только умеет
кэшировать кусками, в разные кэш-бины, с различными метками и для различных
ситуаций, но и умеет гибко инвалидировать.
...
есть [ленивые билдеры](/blog/154) — это как lazy load для картинок, только на
бэкенде и для PHP. Не устраивает его работа? Пришла в голову более гениальная
реализация или определённые элементы надо отдавать несколько иначе? Не
проблема, [стратегии рендера плейсхолдеров](/blog/213) полностью передают
процесс под контроль. Drupal может сам определять когда и где необходимо
задействовать данное поведение, без вмешательства пользователя или разработчика.
...
Многие решения в Drupal основаны на [системе плагинов](/blog/134). Плагин — как
правило это класс с аннотацией реализующий конкретный тип плагинов. Но они также
могут быть объявлены в YAML файлах и функциях, а если потребуется, можно
объявить собственные Discovery объекты и добавить возможность описывать плагины
хоть в JSON, хоть в Markdown. Поведение и применение сильно зависит от
назначения плагина.

Также эта система предоставляет побочные инструменты, которые позволяют решать
различные кейсы с их применением, например,
можно [генерировать плагины при помощи деритив](/blog/203) если их код
...
можете объявлять свои [REST ресурсы](/blog/165). Если хотите что-то иное, можете
...
полностью менять то, [как будет отвечать Drupal](/blog/208) для различных
ситуаций.
...
из коробки, но также можно [создавать свои собственные](/blog/132).
...
Также имеется механизм [создания псевдо-полей](/blog/177), это такие
...
можно [написать свою логику определения языка](/blog/153).

Всё работает, ЧПУ будут какие хотите, никаких конфликтов и проблем. Для кастома
можно просто и беззаботно [деплоить переводы](/blog/207).
...
систем ролей и прав доступа, до более [гибких и тонких настроек](/blog/63) при
помощи кода.

Всё меняется, настраивается и переопределяется. Не устраивает или не подходит
аутентификация по логину и паролю?
Можно [добавить новый способ аутентификации](/blog/166) с любой логикой, которая
необходима. И все остальные подсистемы будут это учитывать.
...
изменения. [Всё это моментально и бесшовно](/blog/186) для пользователей.
...
есть [события Symfony](/blog/170) и [хуки](/blog/184). Конечно, есть и другие
...
- **Обработчики изображений:** Плагины, которые предоставляют интеграцию с любым
  ...
  PHP. Вы можете [подключать ImageMagick](/blog/139) и прочие инструменты.
...
- [Queue API](/blog/79) — позволяет программно создавать и управлять очередями.
  Есть специальный тип плагина [@QueueWorker](/blog/200) который позволяет
  производить обработку в формате ООП по крону, или не по крону. Эта связка
  может работать полностью автономно, написали и забыли.
- [Batch API](/blog/192) — надстройка над Queue API, которая имеет UI и
...
будут [отсылать письма](/blog/214).
...
[Токен](/blog/191) — это специально отформатированная строка, которая заменяется
...
- [Система фильтрации и обработки текста](/blog/76), позволяющая проводить
...
- [Блокировщики состояний](/blog/211) для предотвращения ситуаций приводящих к
  сбою или дублированию операций.
- [Tour API](/blog/78) для создания простеньких обучающих инструкций для
...
- Множество инструментов которые помогу с SEO оптимизацией. Например,
  ...
  гнуть [хлебные крошки](/blog/129).
- **Logging API** для логирования различных происшествий в системе или
  произведенных действий.
- [State API](/blog/74) для хранения различных состояний. Это что-то типа
  переменных которые хранятся в БД.
- [Temp Store API](/blog/188) для создания временных хранилищ данных, общих для
  проекта или специфичных для конкретного пользователя, которые в дальнейшем
  самоуничтожаются.
- [Middleware API](/blog/187) позволяющий производить операции связанные с
  запросами на очень ранних этапах.
